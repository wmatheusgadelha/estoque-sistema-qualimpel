...
def get_current_user(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise credentials_exception
    user = db.query(EstoqueUser).filter(EstoqueUser.email == email, EstoqueUser.is_active == True).first()
    if user is None:
        raise credentials_exception
    return user
...
def require_gestor_or_admin(current_user=Depends(get_current_user)):
